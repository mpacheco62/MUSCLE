...
module muscle_tensor_iden_4o4t
    ...
    !!
    !! Similar to other non-scaled identity types (`iden_2O`, `iden_4O3T`), the `iden_4O4T`
    ...
    !! ### Derived Type:
    !!
    !! - `iden_4O4T`: Represents the standard 3D fourth-order symmetric identity tensor \(\mathbf{I}^{sym}\).
    !!                It contains no data components and has an implicit scale factor of 1.0.
    !!
    !! ### Operators:
    !!
    !! The following operators are overloaded for interactions involving `iden_4O4T`:
    !!
    !! - `*`: Multiplication:
    !!     - `iden_4O4T * real(real64)`: Multiplies the identity by a scalar. Returns `iden_4O4TS` with `val = scalar`.
    !!     - `real(real64) * iden_4O4T`: Multiplies a scalar by the identity. Returns `iden_4O4TS` with `val = scalar`.
    !! - `/`: Division:
    !!     - `iden_4O4T / real(real64)`: Divides the identity by a scalar. Returns `iden_4O4TS` with `val = 1.0D0 / scalar`.
    !!
    !! Usage
    !! -----
    !!
    ...
    !!   implicit none
    !!
    !!   type(iden_4O4T) :: id4_sym_tensor   ! Represents I^sym
    !!   type(iden_4O4TS) :: scaled_id4_sym
    ...
    public :: operator(*)
    interface operator (*)
        module procedure mul_I4O4T_real64
        module procedure mul_real64_I4O4T
    end interface

    public :: operator( / )
    interface operator ( / )
        module procedure div_I4O4T_real64
    end interface

contains

    pure function mul_I4O4T_real64(I2, a) result(res)
        implicit none
        type(iden_4O4T), intent(in) :: I2
        real(real64), intent(in) :: a
        type(iden_4O4TS) :: res
        res%val = a 
    end function mul_I4O4T_real64

    pure function mul_real64_I4O4T(a, I2) result(res)
        implicit none
        type(iden_4O4T), intent(in) :: I2
        real(real64), intent(in) :: a
        type(iden_4O4TS) :: res
    ...
    pure function div_I4O4T_real64(I2, a) result(res)
        implicit none
        type(iden_4O4T), intent(in) :: I2
        real(real64), intent(in) :: a
        type(iden_4O4TS) :: res
