module mod_iden_4O3TS
    ...
    public :: operator(*)
    interface operator (*)
        module procedure mul_I4O3TS_real64
        module procedure mul_real64_I4O3TS
    end interface

    public :: operator( / )
    interface operator ( / )
        module procedure div_I4O3TS_real64
    end interface

    contains

    pure function mul_I4O3TS_real64(IMod, a) result(res)
        implicit none
        type(iden_4O3TS), intent(in) :: IMod
        real(real64), intent(in) :: a
        type(iden_4O3TS) :: res
        res%val = IMod%val * a 
    end function mul_I4O3TS_real64

    pure function mul_real64_I4O3TS(a, IMod) result(res)
        implicit none
        type(iden_4O3TS), intent(in) :: IMod
        real(real64), intent(in) :: a
        type(iden_4O3TS) :: res
        res%val = IMod%val * a 
    end function mul_real64_I4O3TS

    pure function div_I4O3TS_real64(IMod, a) result(res)
        implicit none
        type(iden_4O3TS), intent(in) :: IMod
        real(real64), intent(in) :: a
        type(iden_4O3TS) :: res
