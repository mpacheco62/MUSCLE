module mod_iden_3D4O3T
    ...
    !!
    !! Similar to the second-order identity `iden_3D2O`, the `iden_3D4O3T` type defined here
    ...
    !! corresponding scaled identity tensor type, `iden_3D4O3TS` (defined in `mod_iden_3D4O3TS`),
    ...
    !! ### Derived Type:
    !!
    !! - `iden_3D4O3T`: Represents the standard 3D fourth-order identity tensor of type 3, \(\delta_{ij}\delta_{kl}\).
    !!                It contains no data components and has an implicit scale factor of 1.0.
    !!
    !! ### Operators:
    !!
    !! The following operators are overloaded for interactions involving `iden_3D4O3T`:
    !!
    !! - `*`: Multiplication:
    !!     - `iden_3D4O3T * real(real64)`: Multiplies the identity by a scalar. Returns `iden_3D4O3TS` with `val = scalar`.
    !!     - `real(real64) * iden_3D4O3T`: Multiplies a scalar by the identity. Returns `iden_3D4O3TS` with `val = scalar`.
    !! - `/`: Division:
    !!     - `iden_3D4O3T / real(real64)`: Divides the identity by a scalar. Returns `iden_3D4O3TS` with `val = 1.0D0 / scalar`.
    !!
    !! Usage
    !! -----
    !!
    !! ```fortran
    !! program example_iden_3d4o3t_usage
    !!   use mod_iden_3D4O3T
    !!   use mod_iden_3D4O3TS
    !!   use iso_fortran_env, only: real64
    !!   implicit none
    !!
    !!   type(iden_3D4O3T) :: id4_tensor3     ! Represents delta_ij * delta_kl
    !!   type(iden_3D4O3TS) :: scaled_id4_3t
    ...
    !!
    !! end program example_iden_3d4o3t_usage
    !! ```
    !!
    ...
    type, public :: iden_3D4O3T
        ...
        !! in a scaled identity tensor of type 3 (`iden_3D4O3TS`), which explicitly stores a scaling factor.
        !!
        !! For more information see [[tensors_types]]
