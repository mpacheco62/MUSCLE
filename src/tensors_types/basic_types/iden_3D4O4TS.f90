module mod_iden_3D4O4TS
    ...
    !!
    !! Similar to `iden_3D2OS`, the `iden_3D4O4TS` type explicitly stores the scaling factor \(c\)
    !! in its `val` component. This contrasts with the non-scaled symbolic type `iden_3D4O4T`.
    ...
    !! result in another `iden_3D4O4TS` object.
    !!
    !! Public Entities
    !! ---------------
    !!
    !! ### Derived Type:
    !!
    !! - `iden_3D4O4TS`: Represents the scaled 3D fourth-order symmetric identity tensor \(c \cdot \mathbf{I}^{sym}\).
    !!     - Component: `val :: real(real64)` - Stores the scaling factor \(c\).
    !!
    !! ### Operators:
    !!
    !! The following operators are overloaded for interactions involving `iden_3D4O4TS`:
    !!
    !! - `*`: Multiplication:
    !!     - `iden_3D4O4TS * real(real64)`: Multiplies the scaled identity by a scalar. Returns `iden_3D4O4TS` with `val = IMod%val * a`.
    !!     - `real(real64) * iden_3D4O4TS`: Multiplies a scalar by the scaled identity. Returns `iden_3D4O4TS` with `val = a * IMod%val`.
    !! - `/`: Division:
    !!     - `iden_3D4O4TS / real(real64)`: Divides the scaled identity by a scalar. Returns `iden_3D4O4TS` with `val = IMod%val / a`.
    !!
    !! Usage
    !! -----
    !!
    !! ```fortran
    !! program example_iden_3d4o4ts_usage
    !!   use mod_iden_3D4O4TS
    !!   use iso_fortran_env, only: real64
    !!   implicit none
    !!
    !!   type(iden_3D4O4TS) :: scaled_id4_sym1, scaled_id4_sym2
    ...
    !!
    !! end program example_iden_3d4o4ts_usage
    !! ```
    !!
    ...
    type, public :: iden_3D4O4TS
        ...
        !! This contrasts with the symbolic, non-scaled `iden_3D4O4T` type.
        !!
        !! For more information see [[tensors_types]]
        real(real64) :: val
