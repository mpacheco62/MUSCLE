program test_I3D2OMod
    ...
    call test_iden_3D2OMod_sum(passed)
    if (.not. passed) STOP 1

    call test_iden_3D2OMod_sub(passed)
    if (.not. passed) STOP 2

    call test_iden_3D2OMod_mul(passed)
    if (.not. passed) STOP 3

    call test_iden_3D2OMod_div(passed)
    ...
    print*, "Hola!", passed
    STOP 0
end program test_I3D2OMod


subroutine test_iden_3D2OMod_sum(passed)
    use, intrinsic :: iso_fortran_env
    use tensors_types
    implicit none
    logical, intent(out) :: passed
    type(iden_3D2Omod) :: to_test1, to_test2
    type(iden_3D2Omod) :: temp
...
subroutine test_iden_3D2OMod_sub(passed)
    use, intrinsic :: iso_fortran_env
    use tensors_types
    implicit none
    logical, intent(out) :: passed
...
subroutine test_iden_3D2OMod_mul(passed)
    use, intrinsic :: iso_fortran_env
    use tensors_types
    implicit none
    logical, intent(out) :: passed
    type(iden_3D2Omod) :: to_test1
    type(iden_3D2Omod) :: temp
    real(real64), parameter :: eps=1e-15
...
subroutine test_iden_3D2OMod_div(passed)
    use, intrinsic :: iso_fortran_env
    use tensors_types
    implicit none
    logical, intent(out) :: passed
